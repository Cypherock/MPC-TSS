...
void gen_polynomial_coeff(const uint8_t coeff_count,
  ...
  for (int i = 0; i < coeff_count;) {
    ...
    crypto_random_generate(coefficient, sizeof(coefficient));
...
static void evaluate_lagarange_term(const ecdsa_curve *curve,
...
// interplate threshold+1 points on polynomial to evaluate polynomial at interpolate_point
...
void lagarange_exp_interpolate(const ecdsa_curve *curve,
  ...
  for (int i = 0; i <= threshold; i++) {
    evaluate_exp_lagarange_term(curve, points[i], x_cords[i], interpolate_point,
                                threshold, &term);
